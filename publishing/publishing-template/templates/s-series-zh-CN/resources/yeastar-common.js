jQuery(document).ready( function() {
	...
	function setPdfName() {
	    ...
	    if(pageDocTitle > 0) {
			...
			var faq = jQuery(".hidden-print li:nth-child(2) a").attr("href").replace(/[-]/g,"").indexOf("faq");
	    	if(releaseNotes != -1 || references != -1 || faq != -1) {
		    ...
		    for (var a = 0; a < parentEle.length; a++) {
		    	...
		    	if(e.length > 0) {
					...
					var faq = e.find(".wh_tile_title .topicref a").attr("href").replace(/[-]/g,"").indexOf("faq");
		    		if(releaseNotes != -1 || references != -1 || faq != -1) {
